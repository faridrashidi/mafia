...
export default {
  ...
  mutations: {
    SET_GAME_SETTINGS: (state, settings) => {
      ...
      state.gameSettings.alivePeople = state.gameSettings.selectedRoles.filter(
        role => !role.status.dead
      ).length;
      state.gameSettings.deadPeople = state.gameSettings.selectedRoles.filter(
        role => role.status.dead
      ).length;
      state.gameSettings.aliveMafia = state.gameSettings.selectedRoles.filter(
        role => !role.status.dead && role.mafia
      ).length;
      state.gameSettings.aliveCitizens = state.gameSettings.selectedRoles.filter(
        role => !role.status.dead && !role.mafia
      ).length;
      state.gameSettings.aliveSolo = state.gameSettings.selectedRoles.filter(
        role => !role.status.dead && !role.mafia && role.solo
      ).length;
      state.gameSettings.deadMafia = state.gameSettings.selectedRoles.filter(
        role => role.status.dead && role.mafia
      ).length;
      state.gameSettings.deadCitizens = state.gameSettings.selectedRoles.filter(
        role => role.status.dead && !role.mafia
      ).length;
      // End Game Rules
      const hasFinishStopper = state.gameSettings.selectedRoles.filter(
        role => role.solo && role.status.finishStopper && !role.status.dead
      );
      const mafiaOdds = state.gameSettings.selectedRoles.filter(
        role => !role.status.dead && role.mafia && !role.status.traitor
      ).length;
      ...
      if (state.gameSettings.soloWins && !state.gameSettings.reviewGame) {
        ...
        state.gameSettings.alivePeople / 2 <= mafiaOdds &&
        hasFinishStopper.length === 0 &&
        state.gameSettings.gameStarted &&
        !state.gameSettings.reviewGame
      ) {
        finishGame("mafia");
      } else if (
        mafiaOdds === 0 &&
