  ...
  <div class="action-box">
    <div class="player-box-holder has-small-bottom-margin">
      <div class="player-box">
        <img :src="getImg('/roles', player.icon)" :alt="$t('god.playerIconAlt')" />
        <h4
      ...
      <div class="player-box">
        <template v-if="Object.keys(dashboard.targetData).length">
          <img :src="getImg('/roles', dashboard.targetData.icon)" :alt="$t('god.playerIconAlt')" />
          <h4
            class="has-xsmall-top-margin"
            :class="{
              'mafia-role': dashboard.targetData.mafia,
              'citizen-role': !dashboard.targetData.mafia && !dashboard.targetData.solo,
              'solo-role': !dashboard.targetData.mafia && dashboard.targetData.solo
            }"
          >
            {{ dashboard.targetData.info[currentLang].name }}
            <span>( {{ dashboard.targetData.player }} )</span>
          </h4>
          <div class="other-targets">
            <img
    ...
    <div v-if="player.ability.chooseBoolean" class="question">
      <div class="checkbox-box yes">
        <input v-model="useAbility" id="use_ability" type="checkbox" name="question" />
        <label for="use_ability">
          <span>
            {{ $t("god.useAbility") }}
          </span>
        </label>
    ...
    <template v-else>
      <div v-for="number in player.status.targetNumbers" :key="number" class="select-targets">
        <label :for="`action_target_${number}`">
          <span v-if="number === 1">
        ...
        <select v-model="targets[number - 1]" name="action_target" @change="findTarget(targets[0])">
          <option :value="null" disabled>
            {{ $t("god.selectPlaceholder") }}
          </option>
          <option v-for="(person, index) in checkGroupToSelectTarget(player)" :key="index">
            {{ person.player }}
          </option>
        </select>
        <select
    ...
    <!-- Action Buttons -->
    <BaseButton @clicked.once="executeAction(player, targets, playerIndex, givenAbilities)">
      {{ $t("god.confirmButton") }}
    </BaseButton>

    <template v-if="!force">
      <BaseButton class="danger" @clicked="alertBox = true">
        {{ $t("god.skipButton") }}
      </BaseButton>
      <!-- Alert Box -->
      <Overlay
      ...
      >
        ...
        <template>
          ...
          <BaseButton class="green" @clicked="skipAction(playerIndex)">
            <span>
              {{ $t("god.skipButton2") }}
            </span>
          </BaseButton>
          <BaseButton class="danger" @clicked="alertBox = false">
            <span>
              {{ $t("god.cancelButton") }}
            </span>
          </BaseButton>
