<template>
  <div class="revenge-alert-box">
    <p>
      <strong class="has-left-right-margin">
        {{ dashboard.avenger }}
      </strong>
      {{ $t("god.avengerText") }}
    </p>
    ...
    <BaseButton @clicked="killByVote(dashboard.targetData.player, true)">
      {{ $t("god.confirmButton") }}
    </BaseButton>
    <BaseButton class="danger" @clicked="deadWatcher(false)">
      {{ $t("god.skipButton") }}
    </BaseButton>
  </div>
</template>
