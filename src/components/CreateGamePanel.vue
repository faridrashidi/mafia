<template>
  <div class="create create-game-panel">
    <!-- Setup Game Settings -->
    <PageBox>
      <SelectScenario />
    </PageBox>
    <PageBox>
      <SelectNumbers total-players />
    </PageBox>
    <PageBox>
      <SelectNumbers />
    </PageBox>
    ...
    <AllCharacters />
    <!-- Start Game Button -->
    <BaseButton class="start-bttn has-top-margin primary" @clicked="toggleOverlay(true)">
      <span>
        {{ $t("pages.home.start") }}
      </span>
    </BaseButton>
    ...
    >
      ...
      <template v-if="isValid">
        ...
        />
        <BaseButton class="settings-bttn danger" @clicked="toggleOverlay(false)">
          <span>
      ...
      <template v-else>
        <NoteBox />
        <Table :table-data="gameSettings.fMafias" class="mafia-table" />
        <Table :table-data="gameSettings.fCitizens" class="citizen-table" />
        <BaseButton class="start-bttn primary" @clicked="startGame()">
          <span>
            {{ $t("pages.home.start") }}
          </span>
        </BaseButton>
...
export default {
  ...
  computed: {
    isValid() {
      ...
      if (
        this.gameSettings.selectedMafia == this.gameSettings.mafia &&
        this.gameSettings.selectedCitizen == this.gameSettings.citizen
      ) {
        return false;
      }
      return true;
