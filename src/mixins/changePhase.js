    ...
    logEventsToUser() {
      ...
      this.gameSettings.selectedRoles.forEach(role => {
        ...
        if (role.status.recentlyDead) {
          const logNote = `<span class='last-log red-bg dead-icon'><i>${role.player}</i> ${this.$t(
            "god.logDeadText"
          )}</span><em>${currentIdentity}</em>`;
          resultForDiscord += `
        ...
        if (role.status.recentlyRevived) {
          const logNote = `<span class='last-log green-bg revived-icon'><i>${
            role.player
          }</i> ${this.$t("god.logRevivedText")}</span><em>${currentIdentity}</em>`;
          resultForDiscord += `
        ...
        if (role.status.recentlySilenced) {
          const logNote = `<span class='last-log blue-bg silence-icon'><i>${
            role.player
          }</i> ${this.$t("god.logSilencedText")}</span><em>${currentIdentity}</em>`;
          resultForDiscord += `
