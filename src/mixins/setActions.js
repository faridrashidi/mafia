export default {
  methods: {
    setActionsByPriority() {
      const actions = [];
      const killersInGame = this.gameSettings.selectedRoles.filter(
        role => role.ability.killer && !role.status.dead && role.mafia
      ).length;
      const mafiaInGame = this.gameSettings.aliveMafia;
      if (killersInGame === 0 && mafiaInGame !== 0) {
        const potentialKillers = this.gameSettings.selectedRoles.filter(
          role => role.potentialKiller && !role.status.dead
        );
        const sortedPotentialKillers = [
          ...potentialKillers.slice().sort((a, b) => (a.killPriority > b.killPriority ? 1 : -1))
        ];
        this.gameSettings.selectedRoles.forEach((role, index) => {
          if (
            sortedPotentialKillers.length > 0 &&
            role.player === sortedPotentialKillers[0].player
          ) {
      ...
      this.gameSettings.selectedRoles.forEach(role => {
        ...
        if (
          !role.status.dead &&
          role.status.hasAction &&
          !role.hasDoneAction &&
          !role.ability.reviver &&
          !role.ability.resurrect
        ) {
          actions.push(role);
        }
        ...
        if (
          ...
          !role.hasDoneAction &&
          role.ability.reviver &&
          this.gameSettings.deadPeople !== 0
        ) {
          actions.push(role);
        }
        ...
        // Push all roles that has action used or not but dead
        if (
          role.status.dead &&
          (role.status.hasAction || (!role.status.hasAction && role.hasDoneAction))
        ) {
          actions.push(role);
        }
