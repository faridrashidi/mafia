export default {
  methods: {
    ...
    activateLink(name) {
      const linkTarget = this.gameSettings.selectedRoles.filter(
        role => role.player !== name && role.status.link
      )[0];
      this.setStatus(name, {
        link: false
      });
    ...
    kill(target, killType, player) {
      let checkLoyalty = [];
      if (player) {
        checkLoyalty = this.gameSettings.selectedRoles.filter(
          role =>
            player.ability.loyalty &&
            role.player === target &&
            player.mafia === role.mafia &&
            player.solo === role.solo
        );
      }
      // Check If DeadMagnet is in the game, He should be the target
      const checkDeadMagnet = this.gameSettings.selectedRoles.filter(
        role => role.status.deadMagnet && !role.status.dead
      );
      const checkKillerPlayer = this.gameSettings.selectedRoles.filter(
        role => role.player === target && role.ability.killer && role.mafia && !role.status.dead
      );
